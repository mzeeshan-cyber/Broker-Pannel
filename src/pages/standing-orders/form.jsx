...
export default function AddTripForm({ mappedPatients }) {
    ...
    const generateDates = () => {
        ...
        while (start.isBefore(end) || start.isSame(end, 'day')) {
            const day = start.day(); // 0–6 (Sun–Sat)

            if (repeatDays.includes(day)) {
                all.push(start.format('YYYY-MM-DD'));
            }
                        ...
                        <MainCard title="Recurring Schedule" sx={{ mb: 3 }}>
                            <LocalizationProvider dateAdapter={AdapterDayjs}>
                                <Grid container spacing={2}>
                                        ...
                                        <Stack direction="row" spacing={1} flexWrap="wrap">
                                            {exceptDates.map((d, i) => (
                                                <Button
                                                    ...
                                                    variant="outlined"
                                                    onClick={() => {
                                                        setExceptDates(prev => prev.filter((_, index) => index !== i));
                                                    }}
                                                >
                                    ...
                                    {/* Generated Dates JSON */}
                                    <Grid item xs={12}>
                                        <Typography variant="subtitle2">Generated Trip Dates</Typography>
                                        <Box sx={{ bgcolor: isDark ? '#111' : '#f5f5f5', p: 2, borderRadius: 2 }}>
                                            <pre>{JSON.stringify(finalDates, null, 2)}</pre>
                                        </Box>
                                    </Grid>
