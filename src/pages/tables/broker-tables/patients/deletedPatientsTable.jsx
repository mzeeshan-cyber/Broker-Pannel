...
function EditAction({ row, table }) {
  ...
  const restorePatient = async (id) => {
    const response = await fetcherPatch([`/restore-patient/${id}`])
    if (response) {
      openSnackbar({
        open: true,
        message: response.message || 'Patient restored successfuly!',
        variant: 'alert',
  ...
  return (
    <Stack direction="row" spacing={1} alignItems="center">
      ...
      <TransitionsModal openModal={openModal} setOpenModal={setOpenModal} title="Restore Patient" handleSubmit={() => restorePatient(row.original.id)} btnText='Yes' >
        <Typography id="modal-modal-description">Are you sure, you want to restore this patient?</Typography>
      </TransitionsModal>
    </Stack>
  );
}
...
function ReactTable({ defaultColumns, restoreMultiplePatient, pagination }) {
  const dispatch = useDispatch();
  const deletedPatientData = useSelector(state => state?.patient?.deletedPatientsData)
  let data = deletedPatientData;
  ...
  const handleChangePagination = async (event, value) => {
    ...
    const response = await fetcher([`/deleted-patients?page=${eventValue ? eventValue : value}`]);
    if (response.status === true) {
      dispatch(deletedPatientsData(response?.data?.data))
      openSnackbar({
        open: true,
  ...
  const handleChange = async (event) => {
    ...
    const response = await fetcher([`/deleted-patients?per_page=${per_page}`]);
    if (response.status === true) {
      dispatch(deletedPatientsData(response?.data?.data))
      openSnackbar({
        open: true,
    ...
    <MainCard content={false}>
      ...
      >
        ...
        <Stack direction="row" spacing={2} alignItems="center" sx={{ width: { xs: '100%', sm: 'auto' } }}>
          ...
          >
            {`Restore Patients ${seletedPateints.length > 0 ? `(${seletedPateints.length})` : ''}`}
          </Button>
        </Stack>
      </Stack>
      <TransitionsModal openModal={openModal} setOpenModal={setOpenModal} title="Restore Patients" handleSubmit={() => restoreMultiplePatient(seletedPateints)} btnText='Restore'  >
        <Typography id="modal-modal-description">Are you sure, you want to restore these patient?</Typography>
      </TransitionsModal>
      ...
      <ScrollX>
        ...
        <Box sx={{ p: 2 }}>
          <TablePagination
            totalRecords={pagination?.data?.total}
            page={page}
            pageSize={pageSize}
...
export default function DeletedPatientsTable({ handleDelete, handleUpdate, restoreMultiplePatient, pagination }) {
  ...
  return (
    <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
      <ReactTable {...{ defaultColumns: columns, handleDelete, handleUpdate, restoreMultiplePatient, pagination }} />
    </DndProvider>
  );
}
