...
//assets
import { ArrowDown2, ArrowRight2, CloseCircle, Command, Edit2, Bag, TableDocument, Trash } from 'iconsax-react';
...
import Filters from 'components/pages/patients/filters';
import { userStatuses } from 'constants/constants';
import { openSnackbar } from 'api/snackbar';
import { useDispatch, useSelector } from 'react-redux';
import { updatePatientStatus } from 'store/reducers/patientSlice';
...
function EditAction({ row, table }) {
  const { loading, submitting } = useSelector(state => state.patient)
  ...
  return (
    <Stack direction="row" spacing={1} alignItems="center">
      <Tooltip title='Edit'>
        <IconButton color={'primary'} onClick={() => navigate(`/patients/${row.original.id}/update`)}>
      ...
      <TransitionsModal openModal={openModal} setOpenModal={setOpenModal} title="Delete Patient" handleSubmit={() => table.options.meta.deleteRow(row.original.id)} btnText='Delete' isSubmitting={submitting} loading={loading}>
...
function ReactTable({ defaultColumns, handleDelete, handleUpdate, getPatientsBySearch, handleChangePagination, handleChange, pageSize, page }) {
  const patientStateData = useSelector(state => state?.patient)
  const patientsPaginationData = patientStateData?.patientsPaginationData;
  let data = patientStateData?.patientsData;
    ...
    <MainCard content={false} sx={{ overflow: 'visible !important' }}>
      ...
      >
        ...
        <Stack direction="row" spacing={2} alignItems="center" sx={{ width: { xs: '100%', sm: 'auto' } }}>
          <Filters getPatientsBySearch={getPatientsBySearch} />
          ...
          <Tooltip title='Deleted Patients'>
            <IconButton color={'error'} onClick={() => navigate(`/patients/deleted`)}>
              <Trash
                variant="Bold"
              />
            </IconButton>
      ...
      <ScrollX>
        ...
        <Box sx={{ p: 2 }}>
          <TablePagination
            totalRecords={patientsPaginationData?.total}
            page={page}
            pageSize={pageSize}
...
export default function PatientsTable({ handleDelete, handleUpdate, getPatientsBySearch, handleChangePagination, handleChange, setPageSize, setPage, pageSize, page }) {
  ...
  const changePatientStatus = async (id, status) => {
    try {

      const response = await fetch(`${API_URL}patient/${id}/status`, {
        method: 'PATCH',
        headers: {
      ...
      if (response.status === 200) {
        dispatch(updatePatientStatus({ id, status }));
        openSnackbar({
          open: true,
          message: response.data?.message || "Status successfuly changed",
          variant: 'alert',
          alert: { color: 'success' }
        });
      } else {
      ...
      },
      {
        ...
        cell: ({ row }) => {
          ...
          const handleChange = (event, rowId) => {
            ...
            changePatientStatus(rowId, newValue, dispatch)
          };
          return (
            <Select
  ...
  return (
    <DndProvider backend={isMobile ? TouchBackend : HTML5Backend}>
      <ReactTable {...{ defaultColumns: columns, handleDelete, handleUpdate, getPatientsBySearch, handleChangePagination, handleChange, setPageSize, setPage, pageSize, page }} />
    </DndProvider>
  );
}
