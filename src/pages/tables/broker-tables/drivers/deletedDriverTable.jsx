...
function ReactTable({ defaultColumns, handleDelete, handleUpdate, handleRestoreMultiple }) {
  ...
  const handleChangePagination = async (event, value) => {
    ...
    const response = await fetcher([`/reimbursement-drivers/deleted?patient_id=${patientId.id}&page=${eventValue ? eventValue : value}`]);
  ...
  const handleChange = async (event) => {
    ...
    const response = await fetcher([`/reimbursement-drivers/deleted?patient_id=${patientId.id}&per_page=${per_page}`]);
    if (response.status === true) {
      dispatch(deletedDriversData(response?.data?.data))
      dispatch(resetFilter(false))
      openSnackbar({
...
export default function DeletedDriversTable({ handleDelete, handleUpdate, handleRestoreMultiple }) {
