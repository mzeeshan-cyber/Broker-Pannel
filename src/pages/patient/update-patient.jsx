...
export default function UpdatePatient() {
    ...
    const fields = ["address", "date_of_birth", "gender", "phone_number", "social_security_number", "routing_number", "mobility", "funding_source"];
    ...
    const FetchPatient = async () => {
        try {
            ...
            if (!response.ok) {
                ...
                openSnackbar({
                    open: true,
                    message: errorData ? errorData?.message : 'Patient is not fetched!',
                    variant: 'alert',
    ...
    const UpdatePatient = async (values, { setSubmitting, setErrors }) => {
        try {
            const response = await fetch(`${API_URL}patient/${id}`, {
                method: 'PUT',
                headers: {
            ...
            if (!response.ok) {
                ...
                openSnackbar({
                    open: true,
                    message: errorData ? errorMsg?.message : 'Patient is not updated!',
                    variant: 'alert',
    ...
    return (
        <Formik
            initialValues={{
                ...
                date_of_birth: data?.patient_detail?.date_of_birth,
                gender: data.patient_detail?.gender,
                phone_number: data?.patient_detail?.phone_number,
                social_security_number: data?.patient_detail?.social_security_number,
                mobility: data?.patient_detail?.mobility,
                medicaid_number: data?.patient_detail?.medicaid_number,
                account_number: data?.patient_detail?.account_number,
                routing_number: data?.patient_detail?.routing_number,
                identifier_number: data?.patient_detail?.identifier_number,
                funding_source: data?.patient_detail?.funding_source,
                notes: data?.patient_detail?.notes,
            }}
            ...
            validationSchema={Yup.object().shape({
                ...
                routing_number: Yup.string().matches(/^\d{0,9}$/, "Must be up to 9 digits only"),
            })}
            onSubmit={UpdatePatient}
        >
            {({ errors, handleBlur, handleChange, handleSubmit, isSubmitting, touched, values, setFieldValue }) => (
                <form noValidate onSubmit={handleSubmit}>
                    <MainCard title="Update Patient">
                            ...
                            <Grid item xs={12}>
                                <Stack direction="row" spacing={2} justifyContent="right" alignItems="center" sx={{ mt: 4 }}>
                                    ...
                                    <Button disableElevation disabled={isSubmitting} variant="contained" type='submit' sx={{
                                        ...
                                        {isSubmitting ? <CircularProgress sx={{ height: '20px !important', width: '20px !important', color: 'white' }} /> : 'Update Patient'}
                                    </Button>
                                </Stack>
                            </Grid>
                        </Grid>
